...
def fetch_from_db(start_date="2017-11-10"):
    ...
    WITH 
    NIFTY50 AS (
        SELECT trade_date, adj_close, name 
...
def save_corr_and_plot(name, returns_df):
    ...
    csv_file = os.path.join(OUTPUT_DIR, f"{name}_close_corr.csv")
    png_file = os.path.join(OUTPUT_DIR, f"{name}_close_heatmap.png")
...
def save_rolling_corr(series_a, series_b, name_a, name_b):
    ...
    rc_csv = os.path.join(OUTPUT_DIR, f"rolling_corr_{name_a}_{name_b}_close.csv")
    rc_png = os.path.join(OUTPUT_DIR, f"rolling_corr_{name_a}_{name_b}_close.png")
    rc.to_csv(rc_csv)
...
def run_analysis(start_date="2017-11-10"):
    ...
    close_file = os.path.join(OUTPUT_DIR, "all_closes.csv")
    close_df.to_csv(close_file)
    logging.info(f"Saved closes: {close_file}")

    returns_df = compute_returns(close_df)
    returns_file = os.path.join(OUTPUT_DIR, "all_returns.csv")
